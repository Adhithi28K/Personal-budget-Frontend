...
export const BudgetContextProvider = ({ children }) => {
  ...
  const addCategory = async (category) => {
    try {
      const response = await Axios.post('/BudgetManager/CreateBudgetCategory', {
        budgetId: budget.id,
        name: category.name,
  ...
  const addExpense = async (expense) => {
    try {
      const response = await Axios.post('/BudgetManager/CreateExpense', {
  ...
  const removeCategory = async (id) => {
    try {
      const response = await Axios.post('/BudgetManager/deleteCategory', {
        categoryId: id,
      });
      enqueueSnackbar(response.data.message, { variant: 'success' });
      fetchInitialBudget();
  ...
  const removeExpense = async (id) => {
    try {
      const response = await Axios.post('/BudgetManager/deleteExpense', {
        expenseId: id,
      });
      enqueueSnackbar(response.data.message, { variant: 'success' });
      fetchInitialBudget();
  ...
  const fetchInitialBudget = async () => {
    try {
      const response = await Axios.get('/BudgetManager/AllBudgets');
      let expenses = await Axios.get('/BudgetManager/AllExpense');
      let sortedExpenses = await Axios.get('/BudgetManager/getAllUserExpensesSortedByDate');
      ...
      if (data[0]) {
        ...
        try {
          ...
          const response = await Axios.post('/BudgetManager/CreateInitialBudget', {
            name: 'My Budget',
            totalAmount: 25000,
            startDate: startDate,
